...
static int test_generate_and_validate(void) {
    ...
    json_object_object_add(claims, "sub", json_object_new_string(expected_sub));
...
static int test_invalid_secret(void) {
    ...
    json_object *claims = json_object_new_object();
    json_object_object_add(claims, "sub", json_object_new_string("test-user"));
    ...
    json_object *validated_claims = NULL;
    error = NULL;
    bool valid = jwtc_validate(token, wrong_secret, 0, &validated_claims, &error);
    free(token);
    free(error);
    json_object_put(validated_claims);  

    if (valid) {
        printf("FAIL: Validation should have failed with wrong secret\n");
        return 1;
    }
...
static int test_invalid_format(void) {
    ...
    const char *secret = "simple-secret-key-1234567890abcdef";
    const char *invalid_token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.malformed";  

    json_object *validated_claims = NULL;
    char *error = NULL;
    bool valid = jwtc_validate(invalid_token, secret, 0, &validated_claims, &error);
    free(error);
    json_object_put(validated_claims);
