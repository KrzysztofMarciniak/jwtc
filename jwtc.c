...
static unsigned char* base64url_decode(const char* input, size_t* out_len,
        ...
        int decoded_len =
            EVP_DecodeBlock(decoded, (const unsigned char*)tmp, len + padding);
        if (decoded_len < 0) {
                free(decoded);
                free(tmp);
...
char* jwtc_generate(const char* secret, int expiry_seconds, json_object* claims,
        ...
        char* header_b64        = base64url_encode((unsigned char*)header_json,
                                                   strlen(header_json), error);
        ...
        json_object_object_add(payload, "iat",
                               json_object_new_int64(time(NULL)));
        json_object_object_add(
            payload, "exp", json_object_new_int64(time(NULL) + expiry_seconds));
        const char* payload_json = json_object_to_json_string(payload);
        char* payload_b64 = base64url_encode((unsigned char*)payload_json,
                                             strlen(payload_json), error);
        ...
        if (!HMAC(EVP_sha256(), (const unsigned char*)secret, strlen(secret),
                  (unsigned char*)input, input_len, hmac, &hmac_len)) {
                free(header_b64);
                free(payload_b64);
                free(input);
...
bool jwtc_validate(const char* token, const char* secret,
                   int time_offset_seconds, json_object** claims,
                   char** error) {
        ...
        *signature_b64++ = '\0';
        ...
        unsigned char* header_json =
            base64url_decode(header_b64, &header_len, error);
        if (!header_json) {
                free(header_b64);
                return false;
        }
        ...
        if (!expected_signature_b64 ||
            strcmp(signature_b64, expected_signature_b64) != 0) {
                free(header_b64);
                free(input);
        ...
        unsigned char* payload_json =
            base64url_decode(payload_b64, &payload_len, error);
        if (!payload_json) {
                free(header_b64);
                free(input);
                return false;
